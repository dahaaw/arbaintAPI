...
const {getUyPass, encPass} = require('../../../utils/encryption');

const { ENV, ENV_KEY } = require("../../../config/environment");

// const {getUyPassAdmin, getUyPass, encPass} = require('../../../utils/security');
/**
 ...
 */
const getUser = async (username, passwordInserted) => {
        if(username.includes('@')){
            where = {email : username}
        }else{
            where = {username}
        }
        const uypas = await getUyPass(where);
        const password = encPass(passwordInserted, uypas.salt);
        where.password = password;
        const user = await  db.users.findOne({ where });
        const ret = {id: user.id, role:user.role};
        return ret;
};
